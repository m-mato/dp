...
\begin{enumerate}
  ...
  \end{itemize}
  \item Spracovanie informácií a vytvorenie unikátneho odtalčku requestu
  \item Nájdenie najbližšieho podobného identifikátoru v databázi
  \item Spracovanie výsledu v podobe vzdialenosti odtlačkov a následná reakcia
  \begin{itemize}
    ...
    sa nový záznam. Frekvencia daného odtlačku v databázi je však zvýšená.
...
identifikátorov. Zároveň je pri štarte aplikácie spustné vlákno vykonávajúce
...
zostavený unikátny odtlačok - identifkátor \textit{Unique Footprint - UFoo}.
...
funckie nájde najviac podobný identifikátor. Podľa miery podobnosti ďalej
algoritmus určí či ide o requesty jedného užívateľa, čo spôsobí zvýšenie jeho
frekvencie, alebo ide o nový odtlačok, pre ktorý vytovrí nový záznam v DB.
Jednou z dodatočných funkcionalít je aj upozornenie hostiteľskej aplikácie pri
prekročení limitu frekvencie requestov od jendého užívateľa, čo by mohlo
indikovať napríklad DoS útok.

\section{Štruktúra identifikátoru}
co z http a tcp som pouzil
tcp stream
rozdelenie na static a relation
