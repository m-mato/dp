...
konkrétnom frameworku, či technológií. Po vložení jej závislosti do
...
body pre ľubovoľný z používaných frameworkov a prístupov. Momentálne, na
...
z HTTP requestu je proces ich spracovanie vykonávaný v novom vlákne. Logika
...
Ako bolo popísané vyššie, jedným z atribútov, ktoré sú posudzované pri
...
\textit{n} zachytených paketov v podobe triedy \textit{TCPFootprint} a ich IP
adries.
...
konfirmácie, ktoré sú následne reprezentované triedou \textit{HTTPFootprint}.
Štruktúra tried \textit{HTTPFootprint} a \textit{TCPootprint} je dostupná v
prílohe ??.
...
v databáze. Najskôr pomocou triedy  \textit{Serializer} spracuje informácie do
konečnej podoby - \textit{UFooEntity}, ktorá je reprezentáciou samotného
unikátneho identifikátoru. \textit{UFooEntity} obsahuje všetky extrahované
statické a relačné dáta.

Najdôležitejšou časťou celého procesu je však analýza triedou
\textit{FootprintSimilairtyService}, ktorá implementuje algoritmus hľadania
najbližšieho suseda formálne popísaný v predchádzajúcich častiach práce. Jej
...
Pre účely testovania a kalibrácia parametrov algoritmu obsahuje repozitár okrem
